...
- **`wave-terrain-feedback.ts`** - Pure post-flux terrain feedback: holes pool live water into `puddleDepth` during each wave (routed by the pressure kernel toward the deepest connected hole), and commit silt at wave end via `holeCommit` events. A wet castle cell flags a flood. Consumed by `WaveFieldRuntime` via `WaveDynamicSystem`'s `onResolveCells` hook.
- **`wave-erosion.ts`** - Pure flux-projection erosion: projects each wet cell's velocity onto adjacent wall/tower faces (frontal vs shear), accumulates per-face charge across frames, and emits discrete `eroded` hit counts. Hydrostatic charge is proportional to head above the beach-plane rim (`max(0, groundAt+depth-groundLevelAt)`), so water contained within a trench below the rim does zero hydrostatic damage. Consumed by `WaveFieldRuntime` inside the `onResolveCells` hook.
...
## Work tracking
This repository uses backlog.md as a task tracking tool. 
Read `docs/backlog-instructions.md` before planning or executing any work.
